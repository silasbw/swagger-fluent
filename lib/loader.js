...
class Component {
    ...
    if (options.template) {
      component = function templatedComponent (name) {
        ...
        const namedComponent = new Component({
          ...
          splits: splits,
          parameters: Object.assign({ [options.template]: name }, options.parameters)
        })
  ...
  _addEndpoint (endpoint) {
    ...
    if (component.pathItemObject) {
      console.warn(`Component ${this.getPath()} already has a Path Item Object`)
    }
